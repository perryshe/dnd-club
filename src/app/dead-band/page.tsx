...
export default async function DeadBandPage() {
  ...
  return (
    <div className="min-h-screen text-white">
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-b from-red-950/60 via-red-950/40 to-slate-900" />
      ...
      <main className="container mx-auto px-4 pb-16 space-y-16">
        ...
        <section id="characters">
          <div className="flex items-center justify-between mb-6">
            ...
            {session?.user && (
              <Link href="/dead-band/characters/create" className="flex items-center gap-2 bg-red-600 hover:bg-red-700 px-4 py-2 rounded-lg transition text-sm">
                <Plus size={18} />
                Создать
              </Link>
            )}
            ...
            <div className="space-y-6 relative before:absolute before:left-[15px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-700">
              {statuses.map((s) => (
                <div key={s.id} className="relative pl-10">
                  <div className="absolute left-[9px] top-1.5 w-3 h-3 rounded-full bg-amber-600 border-2 border-slate-900" />
                  <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
                    ...
                    {s.result && (
                      <div className="bg-slate-900/50 rounded-lg p-3 border-l-2 border-amber-600">
                        <span className="text-xs font-bold text-amber-400 uppercase">Результат:</span>
