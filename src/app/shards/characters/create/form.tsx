...
export default function CreateCharacterForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await createCharacter("shards", new FormData(e.currentTarget))
      router.push("/shards")
      router.refresh()
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            name="name"
            required
            className="w-full px-4 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white focus:border-purple-500 outline-none"
          />
        </div>
        <div>
          ...
          {[
            ...
            <div key={s.name}>
              ...
              <input
                ...
                max={30}
                defaultValue={10}
                className="w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white text-center focus:border-purple-500 outline-none"
              />
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full px-4 py-2 rounded-lg bg-slate-800 border border-slate-700 text-white focus:border-purple-500 outline-none resize-none"
        />
      </div>
